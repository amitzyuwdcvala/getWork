...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['signup'])) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            $_SESSION['user'] = [ "username" => $username, "email" => $useremail, "user_type" => $profession ?? null, "profile_img" => $profile_img];
...
// Work requests
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['upload-work'])) {
    $title = $_POST['title'];
    ...
    $stmt = $conn->prepare("INSERT INTO work (user_id, title, description, category, budget, deadline) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param('isssds', $user_id, $title, $description, $category, $budget, $deadline);

    if ($stmt->execute()) {
        $success_message = "Work posted successfully!";
        header("Location: index.php");
    } else {
        $error_message = "Error posting work: " . $stmt->error;
    }
}
?>
